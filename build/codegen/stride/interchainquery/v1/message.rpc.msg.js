...
class MsgClientImpl {
    ...
    submitQueryResponse(request) {
        const data = message_1.MsgSubmitQueryResponse.encode(request).finish();
        const promise = this.rpc.request("stride.interchainquery.v1.Msg", "SubmitQueryResponse", data);
        return promise.then(data => message_1.MsgSubmitQueryResponseResponse.decode(new _m0.Reader(data)));
    }
}
