...
export class QueryClientImpl implements Query {
  ...
  epochInfos(request: QueryEpochsInfoRequest = {
    ...
    const promise = this.rpc.request("lum.network.epochs.Query", "EpochInfos", data);
    return promise.then(data => QueryEpochsInfoResponse.decode(new BinaryReader(data)));
  }
  currentEpoch(request: QueryCurrentEpochRequest): Promise<QueryCurrentEpochResponse> {
    const data = QueryCurrentEpochRequest.encode(request).finish();
    const promise = this.rpc.request("lum.network.epochs.Query", "CurrentEpoch", data);
  ...
  epochInfo(request: QueryEpochInfoRequest): Promise<QueryEpochInfoResponse> {
    ...
    const promise = this.rpc.request("lum.network.epochs.Query", "EpochInfo", data);
